...
C
C     CHANGES FOR VERSION 3.2 (10/26/10 V.R. Lecoustre)
c     ADDITION OF NEW COMMON VARIABLES FOR TRANSFIT COEFFICIENTS
c     ADDITIONS BASED ON HAY WANG MODEL
C
...
C             NFITA - THE MATRIX OF A* FITTING COEFFICIENTS FOR EACH
C                     SPECIES PAIR
C             NFITB - THE MATRIX OF B* FITTING COEFFICIENTS FOR EACH
C                     SPECIES PAIR
C             NFITC - THE MATRIX OF C* FITTING COEFFICIENTS FOR EACH
C                     SPECIES PAIR
...
C
     ...
     6   (IMCWRK(IKTDIF+N-1), N=1,NLITE), (RMCWRK(NTDIF+N-1), N=1,NKT)
      ...
      READ (LINKMC, ERR=999) (RMCWRK(NFITA+N-1), N=1,NK3)
      READ (LINKMC, ERR=999) (RMCWRK(NFITB+N-1), N=1,NK3)
      READ (LINKMC, ERR=999) (RMCWRK(NFITC+N-1), N=1,NK3)
...
C
     ...
     *                RU, PATMOS, SMALL, NKK, NO, NLITE, INLIN, IKTDIF,
     ...
     2                NETA, NDIF, NTDIF, NXX, NVIS, NXI, NCP,
     3                NCROT, NCINT, NBIND, NEOK, NSGM,
     4                NAST, NBST, NCST, NXL, NR, NFITA, NFITB, NFITC,
     5                NWRK, K3
...
C
     ...
     5                NPARK, NFITA, NFITB, NFITC
C
      CALL MCLMDT (P, T, X, NKK, NFITA, NFITB, NFITC, K3, SMALL, 
     1             RMCWRK(NWT), RMCWRK(NEOK),
     2             RMCWRK(NZROT), IMCWRK(INLIN), RMCWRK(NEPS),
     3             ICKWRK, CKWRK, RMCWRK, RMCWRK(NXX), RMCWRK(NVIS),
     4             RMCWRK(NAST), RMCWRK(NBST), RMCWRK(NCST),
     5             RMCWRK(NXI),  RMCWRK(NCP), RMCWRK(NCROT),
     6             RMCWRK(NCINT), RMCWRK(NXL), RMCWRK(NR), 
     7             RMCWRK(NBIND), RMCWRK(NFITA), RMCWRK(NFITB),
     8             RMCWRK(NFITC), IMCWRK(IPVT), DT, COND)
C
      RETURN
      END
C
...
C---------------------------------------------------------------------
C
      SUBROUTINE MCLMDT (P, T, X, KK, NFA, NFB, NFC, KK3, SMALL, WT,
     1                   EOK, ZROT, LIN,
     2                   EPS, ICKWRK, CKWRK, RMCWRK, XX, VIS, ASTAR,
     3                   BSTAR, CSTAR, XI, CPOR, CROTOR, CINTOR, XL,
     4                   R,BINDIF, FITAST, FITBST, FITCST, 
     5                   IPVT, DT, COND)
...
C
     ...
     4          IPVT(*), DT(*),
     5          FITAST(7,KK,*), FITBST(7,KK,*), FITCST(7,KK,*)
C
...
C
	...
	DO 112 K=1,KK
	    KSTART = JSTART + (K-1)*7
	    DO 113 L=1,7
		FITAST(L,J,K) = RMCWRK(KSTART+L-1)
  113       CONTINUE
  112   CONTINUE
  111 CONTINUE 
      ...
      DO 114 J=1,KK
         ...
         DO 115 K=1,KK
            KSTART = JSTART + (K-1)*7
            DO 116 L=1,7
                FITBST(L,J,K) = RMCWRK(KSTART+L-1)
  ...
  114 CONTINUE
      ...
      DO 117 J=1,KK
         ...
         DO 118 K=1,KK
            KSTART = JSTART + (K-1)*7
            DO 119 L=1,7
                FITCST(L,J,K) = RMCWRK(KSTART+L-1)
  119       CONTINUE
  118   CONTINUE
  117 CONTINUE
...
C
         ...
         ASTAR(I,J) = FITAST(1,J,I) + FITAST(2,J,I)*T1 +
     1                FITAST(3,J,I)*T2 + FITAST(4,J,I)*T3 + 
     2                FITAST(5,J,I)*T4 + FITAST(6,J,I)*T5 +
     3                FITAST(7,J,I)*T6
         BSTAR(I,J) = FITBST(1,I,J) + FITBST(2,I,J)*T1 +
     1                FITBST(3,I,J)*T2 + FITBST(4,I,J)*T3 +
     2                FITBST(5,I,J)*T4 + FITBST(6,I,J)*T5 +
     3                FITBST(7,I,J)*T6
         CSTAR(I,J) = FITCST(1,I,J) + FITCST(2,I,J)*T1 +
     1                FITCST(3,I,J)*T2 + FITCST(4,I,J)*T3 +
     2                FITCST(5,I,J)*T4 + FITCST(6,I,J)*T5 +
     3                FITCST(7,I,J)*T6
  100 CONTINUE
C
