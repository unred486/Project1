...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE DMSET2 (NEQ, LID, IWORK)
C***BEGIN PROLOGUE  DMSET2
C***DATE WRITTEN   950830   (YYMMDD)
C   DATE MODIFIED  191209        
C
...
C Add matrix CJ * I_d, and do LU decomposition on blocks. --------------
      ...
      IDIAG = IBD
        DO I = 1,MP
          ...
          IDIAG = IDIAG + (MP + 1)
        END DO
        CALL DGEFA2 (BD(IBD), MP, MP, IPBD(IIP), IER)
...
C
      ...
      IB = 1
      IBD = 1
          CALL DGESL2 (BD(IBD), MP, MP, IPBD(IB), B(IB), 0)
C
      RETURN
