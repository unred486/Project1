...
C   d2mach provides the unit roundoff of the computer.
...
C   DGEFA, dgesl2, dgbfa2, dgbsl2 are LINPACK routines for solving 
C          linear systems (dense or band direct methods).
C   daxpy2, dcopy2, ddot2, DNRM2, dscal2 are Basic Linear Algebra (BLAS)
C          routines.
C
C The routines called directly by DDASPK are:
C   DCNST0, DDAWTS, DINVWT, d2mach, DDWNRM, DDASIC, DDATRP, DDSTP,
C   XERRWD
C
...
C
      ...
      JSKIP = 0
      IF (NIC .EQ. 2) JSKIP = 1
      CALL dcopy2 (NEQ, Y, 1, PHI(1,1), 1)
      CALL dcopy2 (NEQ, YPRIME, 1, PHI(1,2), 1)
C
      IF (ICOPT .EQ. 2) THEN
        CJ = 0.0D0 
      ELSE
...
C
      CALL dcopy2 (NEQ, PHI(1,1), 1, Y, 1)
      CALL dcopy2 (NEQ, PHI(1,2), 1, YPRIME, 1)
      GO TO 200
C
 350  IDID = -12
      RETURN
...
C
C***ROUTINES CALLED
C   DFNRMD, DYYPNW, DCNSTR, dcopy2, XERRWD
C
...
C-----------------------------------------------------------------------
 150  IRET = 0
      CALL dcopy2 (NEQ, YNEW, 1, Y, 1)
      CALL dcopy2 (NEQ, YPNEW, 1, YPRIME, 1)
      FNRM = FNRMP
      IF (KPRIN .GE. 1) THEN
...
C
230      CALL DGEFA2(WM,NEQ,NEQ,IWM(LIPVT),IER)
      RETURN
C
C
...
C
550   CALL dgbfa2 (WM,MEBAND,NEQ,IWM(LML),IWM(LMU),IWM(LIPVT),IER)
      RETURN
C
...
C     For a dense matrix, the LINPACK routine dgesl2 is called.
C     For a banded matrix, the LINPACK routine dgbsl2 is called.
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   dgesl2, dgbsl2
C
...
C     Dense matrix.
C
100   CALL dgesl2(WM,NEQ,NEQ,IWM(LIPVT),DELTA,0)
      RETURN
C
...
400   MEBAND=2*IWM(LML)+IWM(LMU)+1
      CALL dgbsl2(WM,MEBAND,NEQ,IWM(LML),
...
C
        CALL dcopy2 (NEQ,  SAVR, 1, DELTA, 1)
        GO TO 300
        ENDIF
C
      IF (IERNEW .NE. 0) GO TO 380
...
C
C***ROUTINES CALLED
C   DFNRMK, DSLVK, DDWNRM, DLINSK, dcopy2
C
...
C
      CALL dcopy2 (NEQ, DELTA, 1, SAVR, 1)
C
...
C
C***ROUTINES CALLED
C   DFNRMK, DYYPNW, DCNSTR, dcopy2, XERRWD
C
...
C
C***ROUTINES CALLED
C   RES, dcopy2, dscal2, PSOL, DDWNRM
C
...
C-----------------------------------------------------------------------
      CALL dcopy2(NEQ, SAVR, 1, R, 1)
      CALL dscal2 (NEQ, RSQRTN, WT, 1)
      IER = 0
      CALL PSOL (NEQ, T, Y, YPRIME, SAVR, PWK, CJ, WT, WP, IWP,
     *           R, EPLIN, IER, RPAR, IPAR)
      CALL dscal2 (NEQ, SQRTN, WT, 1)
      IF (IER .NE. 0) RETURN
C-----------------------------------------------------------------------
C     Calculate norm of R.
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      ...
      LZ = LDL + NEQ
      CALL dscal2 (NEQ, RSQRTN, EWT, 1)
      CALL dcopy2 (NEQ, X, 1, WM(LR), 1)
      DO 110 I = 1,NEQ
 110     X(I) = 0.D0
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
 ...
 115  CONTINUE
      NRSTS = NRSTS + 1
      IF (NRSTS .GT. 0) CALL dcopy2 (NEQ, WM(LDL), 1, WM(LR),1)
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   PSOL, DNRM2, dscal2, DATV, DORTH, DHEQR, dcopy2, DHELS, daxpy2
C
...
C-----------------------------------------------------------------------
        TEM = 1.0D0/SNORMW
        CALL dscal2 (NEQ, TEM, V(1,LL+1), 1)
 90     CONTINUE
 100  CONTINUE
...
C-----------------------------------------------------------------------
 ...
 220    Z(K) = 0.0D0
      DO 230 I = 1,LL
        CALL daxpy2 (NEQ, R(I), V(1,I), 1, Z, 1)
 230    CONTINUE
      DO 240 I = 1,NEQ
 240    Z(I) = Z(I)/WGHT(I)
...
C
      ...
      INTEGER I, I0
      DOUBLE PRECISION ARG, ddot2, DNRM2, SUMDSQ, TEM, VNRM
C
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      I0 = MAX0(1,LL-KMP+1)
      DO 10 I = I0,LL
        HES(I,LL) = ddot2 (N, V(1,I), 1, VNEW, 1)
        TEM = -HES(I,LL)
        CALL daxpy2 (N, TEM, V(1,I), 1, VNEW, 1)
 10     CONTINUE
C-----------------------------------------------------------------------
...
C***DESCRIPTION
C
C This is similar to the LINPACK routine dgesl2 except that
