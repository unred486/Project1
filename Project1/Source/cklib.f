...
! C*KIN CHANGE 27th APRIL 92.....exponent from 30 to 110
! C*****smallexp
!       SMALL = 1.0E-110
!       BIG   = 1.0E+110
! C*****END smallexp
...
!      1        /1X,'         CHEMKIN-II Version 2.8, January 1991',
! C*****double precision
...
C
      ...
      WRITE (LOUT,15)
   15 FORMAT (/1X,' CKLIB:  Chemical Kinetics Library',
     1        /1X,'         CHEMKIN-II Version 4.10, April 1994',
C*****precision > double
     2        /1X,'         DOUBLE PRECISION')
C*****END precision > double
C*****precision > single
...
C
  ...
  150    CONTINUE
         IF (LFORD) THEN
            ...
            DO 160 K = 1, MXORD
               KSPEC = ICKWRK(IcKOR + MXORD*(N-1) + K - 1)
               ORD   = RCKWRK(IrKOR + MXORD*(N-1) + K - 1)
...
!==============================================================================
      SUBROUTINE CKRHT(RHO,T,Y,ICKWRK, RCKWRK,QCI)
!==============================================================================
      ! Return the heat release rate of the ith reaction
      ! Written by Lecoustre MAY 2008
...
C                   Dimension Y(*) at least KK, the total number of
C                   species.
C     I      - Indice of the reaction considered
C                   Data type - integer
C     ICKWRK - Array of integer workspace.
C                   Data type - integer array
C                   Dimension ICKWRK(*) at least LENIWK.
...
C*****END precision > double
C
!       DIMENSION ICKWRK(*), RCKWRK(*), Y(NKK), QCI(NII),
!      &           Q(NII),H(NKK),NUKI(NKK,NII)
! Note: use fixed dimension in order put Y, QCI, Q, H, NUKI into stack
      DIMENSION ICKWRK(*), RCKWRK(*), Y(500), QCI(2000),
     &           Q(2000), H(500), NUKI(500,2000)     
     
C
      INCLUDE 'ckstrt.h'
     
!     FIRST FIND THE SPECIES INVOLVED IN TH ITH REACTION
!     2nd COMPUTE DELTAHR for the ith REACTION
!     3rd Compute rate of progression QI for the ith reaction
!     Compute QCI=DLTHR*QI      

!      write(*,*) 'Inside CKRHT, point 1'
      KDIM = NKK
      CALL CKNU  (KDIM, ICKWRK, RCKWRK, NUKI)
!      write(*,*) 'Inside CKRHT, point 2'
      CALL CKHML (T, ICKWRK, RCKWRK, H)
!      write(*,*) 'Inside CKRHT, point 3'
      CALL CKQYR (RHO, T, Y, ICKWRK, RCKWRK, Q)
!      write(*,*) 'Inside CKRHT, point 4'
      DO I = 1,NII
        DLTHRI = 0.0
      	DO K = 1,NKK
		DLTHRI = DLTHRI+NUKI(K,I)*H(K)
      	END DO
        QCI(I) = DLTHRI*Q(I)
      END DO
!      write(*,*) 'Inside CKRHT, point 5'
      RETURN
      END


      SUBROUTINE CKCONTYR(RHO,T,Y,ICKWRK,RCKWRK,NETPOSCONT,NETNEGCONT)

!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
!     RETURN THE REACTION INDIVIDUAL CONTRIBUTION IN MOLE/CM3/S TO THE
!     NET PRODUCTION (NETPOSCONT) AND NET DESTRUCTION OF OF SPECIES K.
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C
     ...
     &          Q(NII), CIK(NII)
      ...
      DOUBLE PRECISION :: NETPOSCONT(NII,NKK), NETNEGCONT(NII,NKK)
C
C
      INCLUDE 'ckstrt.h'

C     GET THE RATE OF PROGRESS VARIABLES UNITS : MOLE/CM3/S
      CALL CKQYR(RHO, T, Y, ICKWRK, RCKWRK, Q)

C     INITIALIZE THE VARIABLE NETPOSCONT & NETNEGCONT
      NETPOSCONT = 0.0
      NETNEGCONT = 0.0
...
C     LOOP OVER THE SPECIES
      DO K = 1,NKK
        ...
        DO I = 1,NII
          NETPOSCONT(I,K) =  MAX(CIK(I),0.0)
          NETNEGCONT(I,K) = -MIN(CIK(I),0.0)
        ENDDO
